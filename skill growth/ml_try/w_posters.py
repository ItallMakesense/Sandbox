...
train_data = train_data.drop(['Language', 'Country', 'Poster'], axis=1)

train_data['Rating'] = train_data['Rating'].apply(rates_to_numbers)
train_data['Rating'].fillna(0, inplace=True)

for n in range(1,257):
    train_pics = histogram(n)
    print(train_data.shape, train_pics.shape)
    train_data = pd.concat([train_data, train_pics], axis=1)

# train_data = pd.read_csv("/home/kirgenvall/task_2_data/data.csv")
