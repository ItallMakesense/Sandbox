...
class Model(object):
    ...
    def write(self, namespace, prim_key):
        ...
        for name, value in namespace.items():
            ...
            columns.append(name)
            if name == prim_key:
                column_types.append("%s text primary key" % name)
                continue
            column_types.append("%s text" % name)
        ...
        update_values = ",".join(["%s=\"%s\"" % pair for pair in zip(columns, column_values)])
    ...
    def save(self, prim_key):
        ...
        self.write(self.__dict__, prim_key)
    ...
    def __getattribute__(self, key):
        ...
        if hasattr(v, '__get__'):
            return v.__get__(self, None, type(self))
        return v
